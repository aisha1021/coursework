...
author: "Aisha Malik (CUNY Hunter College)"
date: '`r Sys.time()`'
output:
  html_document:
    toc: yes
...
Load in the `year_counts.tsv` and `total_counts.csv` files. Use the `here()` function around the filename to keep 
things portable.Give the columns of `year_counts.tsv` the names `term`, `year`, `volume`, and `book_count`. Give 
the columns of `total_counts.csv` the names `year`, `total_volume`, `page_count`, and `book_count`. Note that 
column order in these files may not match the examples in the documentation.

```{r load-counts}

year_counts <- read_tsv("C:\\Users\\ds3\\coursework\\week3\\ngrams\\year_counts.tsv", col_names = FALSE)

year_counts <- year_counts |>
  rename(term = X1, year = X2, volume = X3, book_count = X4)
year_counts

total_counts <- read_csv("C:\\Users\\ds3\\coursework\\week3\\ngrams\\total_counts.csv", col_names = FALSE)
head(total_counts)
...
The total number of lines in year_counts.tsv is 'r nrow(year_counts)'.
The total number of lines in total_counts.csv is 'r nrow(total_counts)'.

# Part B

> Recreate the main part of figure 3a of Michel et al. (2011). To recreate this figure, you will need two files: the one you downloaded 
in part (a) and the “total counts” file, which you can use to convert the raw counts into proportions. Note that the total counts file 
has a structure that may make it a bit hard to read in. Does version 2 of the NGram data produce similar results to those presented in 
Michel et al. (2011), which are based on version 1 data?


## Join ngram year counts and totals

Join the raw year term counts with the total counts and divide to get a proportion of mentions for each term normalized by the total 
...
joined <- left_join(year_counts, total_counts, by = "year")
joined <- joined |>
...
Plot the proportion of mentions for the terms "1883", "1910", and "1950" over time from 1850 to 2012, as in the main figure 3a 
of the original paper. Use the `percent` function from the `scales` package for a readable y axis. Each term should have a 
different color, it's nice if these match the original paper but not strictly necessary.
...
Part b plot with proportions shows that there is a decrease after each year mention reaches its peak. However, part d graph that depicts frequency of mentions
indicates an continuous increase even after the peak. This is because the overall total volume of mentions increases over time and therefore
reduces this pattern with the proportions. 

# Part F

> Now, using the proportion of mentions, replicate the inset of figure 3a. That is, for each year between 1875 
and 1975, calculate the half-life of that year. The half-life is defined to be the number of years that pass 
before the proportion of mentions reaches half its peak value. Note that Michel et al. (2011) do something 
more complicated to estimate the half-life—see section III.6 of the Supporting Online Information—but they 
claim that both approaches produce similar results. Does version 2 of the NGram data produce similar results 
to those presented in Michel et al. (2011), which are based on version 1 data? (Hint: Don’t be surprised if 
it doesn’t.)
...
# peak_mentions <- joined |>
#   group_by(year, term) |>
#   summarize(max_value = max(proportion)) |>
#   mutate(peak_divided = max_value / 2) |>

# peak_mentions |> select(term, year, max_value, peak_divided)
...
peak_mentions <- joined |>
  group_by(term) |>
  slice_max(order_by = proportion, n = 1, with_ties = FALSE) |>
  rename(peak_year = year, peak_value = proportion) |>
  ungroup()

peak_mentions
peak_mentions |> select(term, peak_year, peak_value)

peak_mentions <- peak_mentions |>
  filter(grepl(pattern = "^[0-9]{4}$", x = term)) |>
  mutate(numeric_term = as.numeric(term)) |>
  filter(numeric_term >= 1875 & numeric_term <= 1975) |>
  filter(peak_year >= numeric_term) |>
  mutate(half_life = peak_value / 2)
peak_mentions |> select(term, peak_year, peak_value, half_life)
head(peak_mentions)
...
Now, for each year term, find the minimum number of years it takes for the proportion of mentions to decline 
from its peak value to half its peak value. Store this in an intermediate data frame.
...
half_life_df <- joined |>
  ...
  filter(proportion <= peak_value / 2) |>
  group_by(term) |>
  slice_min(order_by = year, n = 1, with_ties = FALSE) |>
  mutate(half_life = year - peak_year) |>
  select(term, peak_year, year, peak_value, proportion, half_life) |>
  ungroup()
...
No, it does not porduce similar results at all. 
